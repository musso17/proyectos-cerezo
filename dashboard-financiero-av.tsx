...
const VistaFinanzas = () => {
  ...
  const [proyectos, setProyectos] = useState([
    {
      ...
      cliente: "Carbono",
      etiqueta: "carbono",
      tipoCliente: "retainer",
      fecha: "2025-02",
      costos: { grabacion: 100, edicion: 150, freelancers: 80, movilidad: 30, equipos: 50 },
      estado: "completado"
    },
    {
      id: 2,
      ...
      tipoCliente: "retainer",
      fecha: "2025-02",
      costos: { grabacion: 80, edicion: 120, freelancers: 50, movilidad: 20, equipos: 30 },
      estado: "completado"
    },
    {
      id: 3,
      nombre: "Video Testimonial",
      cliente: "Carbono",
      etiqueta: "carbono",
      tipoCliente: "retainer",
      fecha: "2025-02",
      costos: { grabacion: 90, edicion: 140, freelancers: 60, movilidad: 25, equipos: 40 },
      estado: "completado"
    },
    {
      id: 4,
      ...
      cliente: "Empresa XYZ",
      etiqueta: "xyz",
      tipoCliente: "variable",
      ingreso: 3500,
      fecha: "2025-01",
      costos: { grabacion: 400, edicion: 600, freelancers: 500, movilidad: 80, equipos: 200 },
      estado: "completado"
    },
    {
      id: 5,
      nombre: "Documental Corto",
      cliente: "ONG ABC",
      etiqueta: "abc",
      tipoCliente: "variable",
      ingreso: 5000,
      fecha: "2025-02",
      costos: { grabacion: 800, edicion: 1200, freelancers: 600, movilidad: 150, equipos: 300 },
      estado: "completado"
    },
    {
      id: 6,
      ...
      ingreso: 2800,
      fecha: "2025-02",
      costos: { grabacion: 350, edicion: 450, freelancers: 300, movilidad: 60, equipos: 150 },
      estado: "completado"
    }
  ]);
  ...
  const [nuevoProyecto, setNuevoProyecto] = useState({
    ...
    ingreso: '',
    fecha: '2025-02',
    costos: { grabacion: '', edicion: '', freelancers: '', movilidad: '', equipos: '' }
  });

  const [nuevoRetainer, setNuevoRetainer] = useState({
    cliente: '',
    pagoMensual: '',
    proyectosMensuales: '',
    etiqueta: ''
  ...
  const calcularCostoTotal = (costos) => {
    return Object.values(costos).reduce((sum, val) => sum + Number(val || 0), 0);
  };
  ...
  const proyectosEnriquecidos = useMemo(() => {
    return proyectos.map(p => {
      ...
      if (p.tipoCliente === 'retainer') {
        const retainer = retainers.find(r => 
          r.etiqueta.toLowerCase() === (p.etiqueta || '').toLowerCase() && r.activo
        );
        if (retainer) {
          ingreso = retainer.pagoMensual / retainer.proyectosMensuales;
        }
      } else {
  ...
  const contadorRetainers = useMemo(() => {
    const mesActual = '2025-02';
    return retainers.filter(r => r.activo).map(r => {
      const proyectosDelMes = proyectosEnriquecidos.filter(
        p => p.etiqueta && p.etiqueta.toLowerCase() === r.etiqueta.toLowerCase() && p.fecha === mesActual
      ).length;
  ...
  const facturacionMensual = useMemo(() => {
    ...
    return Object.keys(porMes)
      ...
      .map(mes => ({
        mes,
        variable: porMes[mes].variable,
        retainer: porMes[mes].retainer,
        total: porMes[mes].variable + porMes[mes].retainer
      }));
  }, [proyectosEnriquecidos, retainers]);
  ...
  const clientesRentables = useMemo(() => {
    ...
    proyectosEnriquecidos
      ...
      .forEach(p => {
        ...
        porCliente[p.cliente].totalFacturado += p.ingreso;
        porCliente[p.cliente].totalCostos += p.costoTotal;
        porCliente[p.cliente].proyectos += 1;
      });

    retainers.filter(r => r.activo).forEach(r => {
      if (!porCliente[r.cliente]) {
        porCliente[r.cliente] = {
          cliente: r.cliente,
          totalFacturado: 0,
          totalCostos: 0,
          proyectos: 0,
      ...
      porCliente[r.cliente].totalFacturado += r.pagoMensual;
      
      const costosRetainer = proyectosEnriquecidos
        .filter(p => p.etiqueta && p.etiqueta.toLowerCase() === r.etiqueta.toLowerCase())
    ...
    return Object.values(porCliente)
      .map(c => ({
        ...c,
        margen: c.totalFacturado - c.totalCostos,
        margenPorcentaje: c.totalFacturado > 0 ? ((c.totalFacturado - c.totalCostos) / c.totalFacturado) * 100 : 0
      }))
      .sort((a, b) => b.totalFacturado - a.totalFacturado);
  }, [proyectosEnriquecidos, retainers]);

  const kpis = useMemo(() => {
    const ingresoRetainer = retainers.filter(r => r.activo).reduce((sum, r) => sum + r.pagoMensual, 0);
    const ingresoVariable = proyectosEnriquecidos
      .filter(p => p.tipoCliente === 'variable')
      .reduce((sum, p) => sum + p.ingreso, 0);
    const totalIngreso = ingresoRetainer + ingresoVariable;
    const totalCostos = proyectosEnriquecidos.reduce((sum, p) => sum + p.costoTotal, 0);
    ...
    return {
      ...
      totalCostos,
      margen,
      margenPorcentaje: totalIngreso > 0 ? (margen / totalIngreso) * 100 : 0,
      clientesActivos: clientesRentables.length,
  ...
  const agregarProyecto = () => {
    ...
    const proyecto = {
      ...
      tipoCliente: nuevoProyecto.tipoCliente,
      etiqueta: nuevoProyecto.cliente.toLowerCase().replace(/\s+/g, ''),
      fecha: nuevoProyecto.fecha,
      estado: 'completado',
      costos: {
        grabacion: Number(nuevoProyecto.costos.grabacion || 0),
        edicion: Number(nuevoProyecto.costos.edicion || 0),
        freelancers: Number(nuevoProyecto.costos.freelancers || 0),
        movilidad: Number(nuevoProyecto.costos.movilidad || 0),
        equipos: Number(nuevoProyecto.costos.equipos || 0)
      }
    };

    if (nuevoProyecto.tipoCliente === 'variable') {
      proyecto.ingreso = Number(nuevoProyecto.ingreso || 0);
    }
  ...
  const agregarRetainer = () => {
    if (!nuevoRetainer.cliente || !nuevoRetainer.pagoMensual || !nuevoRetainer.proyectosMensuales) return;

    const retainer = {
      id: Date.now(),
      cliente: nuevoRetainer.cliente,
      pagoMensual: Number(nuevoRetainer.pagoMensual),
      proyectosMensuales: Number(nuevoRetainer.proyectosMensuales),
      etiqueta: nuevoRetainer.etiqueta || nuevoRetainer.cliente.toLowerCase().replace(/\s+/g, ''),
      activo: true
    };

    setRetainers([...retainers, retainer]);
    setMostrarFormRetainer(false);
    setNuevoRetainer({ cliente: '', pagoMensual: '', proyectosMensuales: '', etiqueta: '' });
  };

  const actualizarRetainer = () => {
    setRetainers(retainers.map(r => r.id === editandoRetainer.id ? editandoRetainer : r));
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => setMostrarFormRetainer(true)}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
            <div className="bg-slate-800 rounded-xl p-6 w-full max-w-md border border-blue-500/30">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setMostrarFormRetainer(false)} className="text-gray-400 hover:text-white">
                  <X size={20} />
                </button>
              </div>
              <div className="space-y-3">
                <input
                  ...
                  value={nuevoRetainer.cliente}
                  onChange={(e) => setNuevoRetainer({...nuevoRetainer, cliente: e.target.value})}
                  className="w-full bg-slate-700 text-white px-4 py-2 rounded-lg"
                />
                <input
                  type="number"
                  placeholder="Pago mensual (S/)"
                  value={nuevoRetainer.pagoMensual}
                  onChange={(e) => setNuevoRetainer({...nuevoRetainer, pagoMensual: e.target.value})}
                  className="w-full bg-slate-700 text-white px-4 py-2 rounded-lg"
                />
                <input
                  type="number"
                  placeholder="Proyectos mensuales"
                  value={nuevoRetainer.proyectosMensuales}
                  onChange={(e) => setNuevoRetainer({...nuevoRetainer, proyectosMensuales: e.target.value})}
                  className="w-full bg-slate-700 text-white px-4 py-2 rounded-lg"
                />
                <input
                  type="text"
                  placeholder="Etiqueta (ej: carbono)"
                  value={nuevoRetainer.etiqueta}
                  onChange={(e) => setNuevoRetainer({...nuevoRetainer, etiqueta: e.target.value})}
                  className="w-full bg-slate-700 text-white px-4 py-2 rounded-lg"
                />
              </div>
              <button
                onClick={agregarRetainer}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 overflow-y-auto">
            <div className="bg-slate-800 rounded-xl p-6 w-full max-w-2xl border border-purple-500/30 my-8">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setMostrarFormProyecto(false)} className="text-gray-400 hover:text-white">
                  <X size={20} />
                </button>
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                />
                <select
                  value={nuevoProyecto.tipoCliente}
                  onChange={(e) => setNuevoProyecto({...nuevoProyecto, tipoCliente: e.target.value})}
                  className="bg-slate-700 text-white px-4 py-2 rounded-lg"
                >
                ...
                {nuevoProyecto.tipoCliente === 'variable' && (
                  <input
                    type="number"
                    placeholder="Ingreso del proyecto (S/)"
                    value={nuevoProyecto.ingreso}
                    onChange={(e) => setNuevoProyecto({...nuevoProyecto, ingreso: e.target.value})}
                    className="col-span-2 bg-slate-700 text-white px-4 py-2 rounded-lg"
                  />
                )}
                <div className="col-span-2">
                  <p className="text-sm text-gray-400 mb-2">Costos de Producción:</p>
                </div>
                <input
                  type="number"
                  placeholder="Grabación"
                  value={nuevoProyecto.costos.grabacion}
                  onChange={(e) => setNuevoProyecto({...nuevoProyecto, costos: {...nuevoProyecto.costos, grabacion: e.target.value}})}
                  className="bg-slate-700 text-white px-4 py-2 rounded-lg"
                />
                <input
                  type="number"
                  placeholder="Edición"
                  value={nuevoProyecto.costos.edicion}
                  onChange={(e) => setNuevoProyecto({...nuevoProyecto, costos: {...nuevoProyecto.costos, edicion: e.target.value}})}
                  className="bg-slate-700 text-white px-4 py-2 rounded-lg"
                />
                <input
                  type="number"
                  placeholder="Freelancers"
                  value={nuevoProyecto.costos.freelancers}
                  onChange={(e) => setNuevoProyecto({...nuevoProyecto, costos: {...nuevoProyecto.costos, freelancers: e.target.value}})}
                  className="bg-slate-700 text-white px-4 py-2 rounded-lg"
                />
                <input
                  type="number"
                  placeholder="Movilidad"
                  value={nuevoProyecto.costos.movilidad}
                  onChange={(e) => setNuevoProyecto({...nuevoProyecto, costos: {...nuevoProyecto.costos, movilidad: e.target.value}})}
                  className="bg-slate-700 text-white px-4 py-2 rounded-lg"
                />
                <input
                  type="number"
                  placeholder="Equipos"
                  value={nuevoProyecto.costos.equipos}
                  onChange={(e) => setNuevoProyecto({...nuevoProyecto, costos: {...nuevoProyecto.costos, equipos: e.target.value}})}
                  className="bg-slate-700 text-white px-4 py-2 rounded-lg col-span-2"
                />
              </div>
              {nuevoProyecto.tipoCliente === 'retainer' && (
                <p className="text-sm text-yellow-300 mt-3">
                  ℹ️ El ingreso se calculará automáticamente según el retainer del cliente
                </p>
              )}
              <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-gradient-to-br from-purple-600 to-blue-600 rounded-xl p-6 shadow-2xl">
            ...
            <p className="text-xs text-purple-200 mt-2">
              Retainer: {currency(kpis.ingresoRetainer)} | Variable: {currency(kpis.ingresoVariable)}
            </p>
          </div>
                ...
                <div key={r.id} className="bg-white/10 backdrop-blur rounded-lg p-4">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      ...
                      <p className="text-blue-100">
                        Contrato: {r.proyectosMensuales} proyectos/mes → {currency(r.pagoMensual)}
                      </p>
                      <p className="text-sm text-blue-200">
                        Por proyecto: {currency(r.pagoMensual / r.proyectosMensuales)}
                      </p>
                    </div>
                    <div className="text-right">
                      <span className="text-4xl font-bold text-white">
                        {r.proyectosRealizados}/{r.proyectosMensuales}
                      </span>
                      <button
                        onClick={() => setEditandoRetainer(r)}
                        className="mt-2 text-white/80 hover:text-white block ml-auto"
                      >
                        <Edit2 size={16} />
                      </button>
                    </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm text-blue-50">
                      ...
                      <span>{r.cumplimiento.toFixed(0)}%</span>
                    </div>
                    ...
                    {r.proyectosPendientes > 0 ? (
                      <p className="text-yellow-300 text-sm flex items-center gap-1 mt-2">
                        <AlertCircle size={16} />
                        Faltan {r.proyectosPendientes} proyecto(s) para cumplir el contrato
                      </p>
                    ) : (
                      <p className="text-green-300 text-sm font-semibold flex items-center gap-1">
                        <CheckCircle size={16} />
                        Contrato mensual completado
                      </p>
                    )}
                  </div>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
            <div className="bg-slate-800 rounded-xl p-6 w-full max-w-md border border-blue-500/30">
              ...
              <div className="space-y-3">
                <input
                  type="text"
                  value={editandoRetainer.cliente}
                  onChange={(e) => setEditandoRetainer({...editandoRetainer, cliente: e.target.value})}
                  className="w-full bg-slate-700 text-white px-4 py-2 rounded-lg"
                />
                <input
                  type="number"
                  value={editandoRetainer.pagoMensual}
                  onChange={(e) => setEditandoRetainer({...editandoRetainer, pagoMensual: e.target.value})}
                  className="w-full bg-slate-700 text-white px-4 py-2 rounded-lg"
                  placeholder="Pago mensual"
                />
                <input
                  type="number"
                  value={editandoRetainer.proyectosMensuales}
                  onChange={(e) => setEditandoRetainer({...editandoRetainer, proyectosMensuales: e.target.value})}
                  className="w-full bg-slate-700 text-white px-4 py-2 rounded-lg"
                  placeholder="Proyectos mensuales"
                />
                <input
                  type="text"
                  value={editandoRetainer.etiqueta}
                  onChange={(e) => setEditandoRetainer({...editandoRetainer, etiqueta: e.target.value})}
                  className="w-full bg-slate-700 text-white px-4 py-2 rounded-lg"
              ...
              <div key={p.id} className="bg-slate-700/50 rounded-lg p-4 hover:bg-slate-700 transition">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      {p.cliente} {p.tipoCliente === 'retainer' && '(Retainer)'} • {p.fecha}
                    </p>
                  </div>
                  <span className={`px-3 py-1 rounded-full text-sm font-semibold ${
                    p.porcentajeMargen > 40 ? 'bg-green-500/20 text-green-400' :
                    p.porcentajeMargen > 20 ? 'bg-yellow-500/20 text-yellow-400' :
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  <div>
                    ...
                    <p className="text-white font-semibold">{currency(p.ingreso)}</p>
                  </div>
                  <div>
                    <p className="text-gray-400">Costos</p>
                    <p className="text-white font-semibold">{currency(p.costoTotal)}</p>
                  </div>
                  <div>
                    <p className="text-gray-400">Margen</p>
                    <p className="text-green-400 font-semibold">{currency(p.margen)}</p>
                  </div>
                </div>
              </div>
            ))}
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-2">
                      <span 
                        ...
                        style={{backgroundColor: COLORS[idx % COLORS.length] + '40', color: COLORS[idx % COLORS.length]}}
                      >
                        {idx + 1}
                      </span>
                      <div>
                        <p className="text-white font-semibold">{c.cliente}</p>
                        <p className="text-xs text-gray-400">
                          {c.tipo === 'retainer' ? (
                            <span className="px-2 py-1 rounded bg-blue-500/20 text-blue-400">Retainer</span>
                          ) : (
                            <span>{c.proyectos} proyectos</span>
                          )}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-white font-semibold">{currency(c.totalFacturado)}</p>
                      <p className="text-xs text-green-400">Margen: {c.margenPorcentaje.toFixed(1)}%</p>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="bg-slate-800 rounded-xl p-6 shadow-2xl border border-purple-500/30">
          ...
          <p className="text-sm text-gray-400 mt-4">
            * Proyección basada en el promedio actual de ingresos mensuales ({currency(kpis.totalIngreso)})
          </p>
        </div>
      </div>
    </div>
